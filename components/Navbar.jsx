	...
	{!sidebar && <span onMouseOver={()=>{setDropdown(true)}} onMouseLeave={()=>{setDropdown(false)}} className="fixed top-4 right-9 z-40" >
			{ dropdown && <div className="absolute right-5 top-5 py-4 bg-white shadow-lg border rounded-md px-5 w-32 z-30">
				<ul>
					...
					<Link href={'/myorders'}><li className="py-1 hover:text-orange-400 cursor-pointer text-sm font-bold">My Orders</li></Link>
        ...
        2. transform transition-transform translate-x-full: Initiates a transformation and transition effect. The translate-x-full moves the cart fully outside the viewport horizontally. */}
			...
			<div ref={ref}
				...
				<div className="flex">
					<Link href={"/checkout"}>
						<button disabled={Object.keys(cart).length == 0} className="flex mr-2 text-white bg-orange-500 border-0 py-2 pr-2 focus:outline-none disabled:bg-orange-300 hover:bg-orange-600 rounded text-sm">
						<BsBagCheckFill className="m-1 text-center" />
						Checkout
						</button>
					</Link>
					<button disabled={Object.keys(cart).length == 0} onClick={clearCart} className="flex mr-2 text-white disabled:bg-orange-300 bg-orange-500 border-0 py-2 pr-2 focus:outline-none hover:bg-orange-600 rounded text-sm">
